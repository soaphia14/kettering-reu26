"""
Testing the FL training process.
...
epochs = 5 # 65
vehicles = 10 # 200
# subEpochs - epochs trained locally per vehicle
# epochs - epochs trained globally
...
mainModel = OBU(7, epochs= subEpochs, gpu = gpu, lr = lr, motors = motors, units = units, outputs = 2)
nextModel = OBU(7, epochs= subEpochs, gpu = gpu, lr = lr, motors = motors, units = units, outputs = 2)
path = f"test/"
...
if not randomVehicles:
    ...
    for vehicle in fed_dataset[500:vehicles+500]: # 10
        ...
        if np.random.randint(0,100) < percEvil:
            models[rcvrID] = OBU(7, epochs = subEpochs, gpu=gpu, lr = lr, motors = motors, units = units, outputs = 2, evil = True)
        else:
            models[rcvrID] = OBU(7, epochs = subEpochs, gpu=gpu, lr = lr, motors = motors, units = units, outputs = 2)
        # Create Slice of dataset
        vehicle = data.DataLoader(data.TensorDataset(vehicle[:,:,3:10].float(), (vehicle[:,:,11] > 0).long()), batch_size=batchSize, shuffle=False, num_workers=16, persistent_workers = True) # type: ignore
        ...
        for vehicle in cars: # 10
            ...
            if rcvrID not in models:
                if np.random.randint(0,100) < percEvil:
                    ...
                    models[rcvrID] = OBU(7, epochs = subEpochs, gpu=gpu, lr = lr, motors = motors, units = units, outputs = 2)
            # Create Slice of dataset
            vehicle = data.DataLoader(data.TensorDataset(vehicle[:,:,3:10].float(), (vehicle[:,:,11] > 0).long()), batch_size=batchSize, shuffle=False, num_workers=16, persistent_workers = True)
            # Add sub - dataset to dataset
            rcvrs.append(rcvrID)
            dataSets[rcvrID]=vehicle
            models[rcvrID].dataset = vehicle
